...
{
    // From KnockoutMVC: 
    // https://github.com/AndreyAkinshin/knockout-mvc
    // Untilities.KnockoutJsModelBuilder
    public static class JsExpressionGenerator
    {
        public class TestClass
        {
            ...
            [Computed]
            [JsGet("Tasks.every(function(t){ return t.Text.length == 0; })")]
            [JsSet("if(value){ Tasks.forEach(function(t){ t.Text = ''; }) }")]
            public bool AreAllEmpty
            {
                get
                {
                    return Tasks.TrueForAll(t => string.IsNullOrEmpty(t.Text));
                }
                set
                {
                    if (value == true)
                    {
                        Tasks.ForEach(t => t.Text = "");
                    }
                }
            }
            ...
            //[Computed]
            //public string FullNameFormated { get { return string.Format("{1}, {0}", FirstName, LastName); } }
            ...
            public TestClass()
            {
                ...
                Tasks = new List<Task>() { new Task() { Text = "A" }, new Task() { Text = "B" } };
            }
        }
        ...
        public static string Test()
        {
            ...
            {
                if (property.HasAttribute<ComputedAttribute>())
                {
                    if (property.HasAttribute<JsGetAttribute>())
                    {
                        sb.AppendFormat("{0}:{2}:()=>{1}", property.Name,
                            property.GetAttribute<JsGetAttribute>().Javascript,
                            property.GetValue(model));
                    }
                    else
                    {
                        sb.AppendFormat("{0}:{2}:()=>{1}", property.Name, PropertyToJsExpression(model, property), property.GetValue(model));
                    }

                    if (property.HasAttribute<JsSetAttribute>())
                    {
                        sb.AppendFormat("set:(value)=>{0}", property.GetAttribute<JsSetAttribute>().Javascript);
                    }
                }
                else
                {
                    sb.AppendFormat("{0}:{1}", property.Name, property.GetValue(model));
                }
        ...
        public static string PropertyToJsExpression(object model, PropertyInfo property)
        {
            ...
            if (property.HasAttribute<ComputedAttribute>())
            {
                var constExpression = Expression.Constant(model);
                var prop = Expression.Property(constExpression, property);
                var decomp = DecompileExpressionVisitor.Decompile(prop);
                jsExpression = ExpressionToString(modelType, decomp);
            }

            return jsExpression;
        }

        private static bool HasAttribute<T>(this PropertyInfo property)
        {
            return property.GetCustomAttributes(typeof(T), false).Any();
        }

        private static T GetAttribute<T>(this PropertyInfo property) where T : class
        {
